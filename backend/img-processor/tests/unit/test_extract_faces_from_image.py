...
def test_extract_faces_from_image_object(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.tasks.extract_faces_from_image.get_uploaded_file_path",
        mock_get_uploaded_file_path,
    )
    monkeypatch.setattr(
        "app.tasks.extract_faces_from_image.detect_faces_from_image",
        mock_detect_faces_from_image,
    )
    monkeypatch.setattr(
        "app.tasks.extract_faces_from_image.store_detected_faces_to_db",
        mock_store_detected_faces_to_db,
    )
    monkeypatch.setattr(
        "app.tasks.extract_faces_from_image."
        "store_detected_faces_images_to_processed_folder",
...
def test_detect_faces_from_image(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.modules.image.services.RetinaFace.detect_faces",
        mock_RetinaFace_detect_faces,
    )
    monkeypatch.setattr(
        "app.modules.image.services.PIL.Image.open",
        mock_PIL_Image_open,
    )
    monkeypatch.setattr(
        "app.modules.image.services.PIL.Image.Image.crop",
        mock_PIL_Image_crop,
    )
...
def test_store_detected_faces_image_info_to_db(client, monkeypatch):
    ...
    monkeypatch.setattr(
        "app.modules.image.services.store_processed_image_info_to_db",
...
def test_store_detected_faces_to_db(client, monkeypatch):
    ...
    for face in faces:
        ...
        assert face_dao.uuid == face["uuid"]
        assert face_dao.file_uuid == face["file_uuid"]
        ...
        assert face_dao.file == face_image
